...
The metrics can be auto-instrumented from Celery using `opentelemetry-instrument`.
...
#    verbosity: detailed
  otlp:
    ...
    tls:
      insecure: false
    headers:
      "signoz-ingestion-key": "grztuiynm7Tw8wroFGWqwCNOKUAXLnQ4Hvar"
service:
  pipelines:
    metrics:
...
You can also manually instrument the Celery metrics using OpenTelemetry SDK.
...
The following code instruments the OpenTelemetry metrics.
...
### Instrumenting Celery

For instrumenting Celery metrics, create a new file, say `instrumentation.py` with the following code:

```
# instrumentation.py
from opentelemetry.instrumentation.celery import CeleryInstrumentor

# Activate Celery instrumentation
...
You can now proceed to your Celery application code. It is necessary that you import the Celery instrumentaion file 
...
### Enabling flower metrics collection (Optional)
...
<figure data-zoomable align="left">
    <img
        src="/img/docs/messaging-queues/celery/dashboard-1.webp"
...
<figure data-zoomable align="left">
    <img
        src="/img/docs/messaging-queues/celery/dashboard-2.webp"
        alt="Celery Dashboard"
    />
<figcaption>
<i>
